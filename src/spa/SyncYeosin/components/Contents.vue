...
export default {
    ...
    methods: {
        submit() {
            this.loading=true;
            this.$http.post(`${baseURI}/sync/cards?startDate=${this.from_date}&endDate=${this.to_date}&chk=1`,{},
            {
                headers: {
                    'authorization2': this.$session.get('yeosin_token')
                }
            })
            .then((res) => {
                if(res.data.status == 100) {
                    ...
                    .then((res) => {
                        if(res.data.status == 100) {
                            ...
                            alert('동기화 실패: 로그인 만료 (문제 해결됨. 다시시도 해주십시오)')
                            location.reload();
                        }
                    })
                    .catch((error) => {
        ...
        update_yeosin_token() {
            ...
            .then((res) => {
                if(res.data.status == 100) {
                    this.loading = false;
                    this.$session.set('yeosin_token', res.data.data.token);
                    alert('동기화 실패: 토큰 만료 (문제 해결됨. 다시시도 해주십시오)');
                    location.reload();
                }
            })
            .catch((error) => {
